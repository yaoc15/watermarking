...
def get_average_pixel():#二值图像统计平均像素点
  ...
  for i in range(0,WORD_BLOCK_NUM):
    if (pixel[i] >= average_pixel*0.5):#原为0.3
    #if ((not word_list[i] in base_word) and (word_list[i] in all_word)):
      #print(word_list[i])
      new_pixel.append(i)
...
def get_print_image_block(information):#将每个字二值化后嵌入水印信息
  ...
  for i in range(len(position_list)):#去除过多的文本
    if (i in pixel_list):
      ...
      #   word_num += 1
      #   flag = 1
      # print(abs(change_pixel/image_binary_pixel))
      ...
      print(i,rest_pixel,image_binary_pixel,change_pixel)
      if (flag == 0):
